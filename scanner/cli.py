...
# Add repo root to path so the sibling intelligent_terminal_ai package imports
# cleanly when running from a source checkout.
...
# Secret-redacting log filter (production hardening)
...
def main():
    ...
    # Production hardening: redact secrets (API keys, tokens, passwords) from all
    # log output so a verbose/debug run never leaks credentials to disk or console.
